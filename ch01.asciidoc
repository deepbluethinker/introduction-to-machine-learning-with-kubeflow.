...
Kubeflow can be run either locally for your development or on a production cluster once you are ready.
...
In addition to being able to switch from a single machine to a distributed cluster, different stages of your machine learning pipeline can request different amounts and types of resources.
For example, with your data preparation step may benefit more from running on multiple machines, and your model training can benefit more from access GPUs.
