...
In the machine learning landscape, there are many diverse tool sets and frameworks. Kubeflow does not seek to reinvent the wheel or provide a "one size fits all" solution - instead, it allows machine learning practitioners to compose and customize their own stacks based on specific needs. Kubeflow is designed to simplify the process of building and deploying machine learning at scale; allowing data scientists to do their jobs without worrying about the "boring parts" of building the infrastructure.

Kubeflow's mission is to make machine learning composable, portable, and scalable. It is designed to leverage the advantages of Kubernetes:
- Easy deployments at production scale
- Portable, cloud-native microservices
- High scalability on demand

Kubeflow is designed to be flexible: the core components can be used indepedently to facilitate specific stages of machine learning, or composed together to form end-to-end pipelines. Let's take a closer look at some of these components.
...
Finding the right set of hyperparameters for your training model can be a daunting task. Traditional methodologies such as grid search can be time consuming and quite tedious. 

Kubeflow provides a component (called "katib") that allows users to perform hyperparameter optimizations easily on Kubernetes clusters. Katib is inspired by Vizier, a black-box optimization framework created at Google. Users can specify a hyperparameter optimization experiment through a custom resource definition, and Kubeflow will take care of the rest - spin up training jobs using different hyperparameters, collect the metrics, and save the results to a model database so their performance can be compared.


==== Notebooks

Interactive notebooks are popular among machine learning practitioners due to their simplicity and portability. Kubeflow allows you spin up instances of Jupyter notebooks that interact with your training clusters seamlessly, so you can launch distributed training jobs directly from your browser.
