...
Kubeflow solves the problem of needing to build portable, reliable, scalable machine learning pipelines and using the results.
...
We briefly look at some alternatives in <<brief_alternatives_to_kubeflow>> for you to explore.
...
Kubeflow is for both data scientists and data engineers looking to build production machine learning pipelines.
Kubeflow packages components for all the different stages of a model's lifecycle (data exploration, feature preparation, model training, model serving, and model testing); allowing users to build integrated end-to-end pipelines from these components.
...
Kubeflow provides a unified system with repeatable components power by containerization and scalability by leveraging Kubernetes.


=== Why Containerization?

Containerization means no more "it worked on my machine" or "oh yeah we you need package this extra package."
...
[TIP]
====
We talk about who this book is for in <<assumptions_about_the_audience>> as well as the required background and resources to get up to speed if you need them.
====
...
[WARNING]
====
These will modify your system packages installing and require root/super user permissions. If you wish you can instead run these commands on another host (e.g. dedicated KF development enviroment).
====
...
[[brief_alternatives_to_kubeflow, brief alternatives to kubeflow section]]
