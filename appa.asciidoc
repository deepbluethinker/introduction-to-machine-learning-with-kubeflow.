...
The cluster generation step currently is hardcoded to assume that you will be using Google's IAP.
...
Once you have kubeflow installed and on your path, as covered in <<who_is_kubeflow_for_ch>>, you can now generate a GCP platform.
...
The name of your Kubeflow application is used in the name of the deployment, which will need to match that of the scope you configured in <<>> (you can also add additional scopes).
