...
Once you have a cluster set up you will need to // TODO: connect to GKE cluster
...
[[using_gke_cluster]]
===== Generating a Kubeflow project with GKE

Now that you have your GKE cluster up and running it's time to deploy Kubeflow to it.
The general set-up instructions in <<general_set_up_kubeflow>> still apply.
Once you have Kubeflow installed, when you go to generate Kubeflow projects, as in
<<create_example_project>> you should replace `--platform none` with `--platform gke`.
...
If you're deploying on GKE and wish to access your Kubeflow installation without having to set up port-forwarding, IAP can be a secure solution.
