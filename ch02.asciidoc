[[simple_training_ch]]
== Old stuff starts here- (Trevo's refactor)
...
If you haven't already cloned the examples that go with the book, let's start by doing that. This repository contains
examples that will be used throughout the course of the book.

```bash
git clone https://github.com/intro-to-ml-with-kubeflow/intro-to-ml-with-kubeflow-examples.git
```

TIP: This example is a slightly modified version of another example. While we will make every
effort to maintain a working version, if you have an issue- check the Issues on this repository.
...
note why are we building a training image?

==== Deploying and Monitoring Training Progress

note w argo

===== And when things go wrong...

note we hope they dont, but ineveitably they will
