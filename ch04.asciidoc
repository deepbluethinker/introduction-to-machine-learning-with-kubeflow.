...
Since the only tool we need is Google's big query, we can use `google/cloud-sdk:247.0.0` as the basis, which includes most of what we need.
...
This job is a bit more complicated so we create a generator
...
These work well when we can find a base image with most of our needs.
In addition to copying values in, you can also run commands to set up your container with `RUN`.
I tend to prefer using Ubuntu based images as it's a distribution I'm familiar with, although others may prefer Alpine since it has less overhead.
